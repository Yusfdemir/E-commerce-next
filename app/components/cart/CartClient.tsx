...
const CartClient = () => {
    const {cartPrdcts,removeFromCart,removeCart,addToBasketIncrease,addToBasketDecrease} = cartContext();
    ...
    <div className='my-3 md:my-10'>
        <PageContainer>
           ...
           <div>
            {
                cartPrdcts.map(cart=>(
                    <div key={cart.id} className='flex items-center gap-3 text-center my-5'>
           ...
           <div className='flex items-center justify-between my-5 py-5 border-t'>
                ...
                <div className='tet-lg md:text-2xl text-orange-600 font-bold'>{cartPrdctsTotal} €</div>
           </div>
        </PageContainer>
    </div>
  )
